...
function SearchProfiles2(props:any) {
    ...
    let chat : Chat = {
        ...
        FromUserID: 0,
        CreatedAt: "",
        PublicProfileRandomNumber: "",
    }
    const [chatMessages, setChatMessages] = useState([chat])
    const [singleChatMessage, setSingleChatMessage] = useState(chat)
    ...
    const addMessage =  (e: SyntheticEvent) => {
        e.preventDefault()
        //console.log(singleChatMessage)
        //const m = chatMessages?.concat(singleChatMessage)
        //setChatMessages([...chatMessages])
        //let m = chatMessages?.concat(singleChatMessage)

        //setChatMessages([m]);
        //setChatMessages( (prevState) => prevState, singleChatMessage )

        //const updatedChatsArray = [...chatMessages, singleChatMessage];
        //setChatMessages([...updatedChatsArray])
        //chatMessages.push(singleChatMessage)
        setChatMessages( [...chatMessages, singleChatMessage] )
        setSingleChatMessage(chat)
        //setSingleChatMessage({})
                                        ...
                                        <div className="col-lg-3 col-md-6 col-sm-8 mt-0 d-flex" key={profile.ID}>
                                            <div className="card mb-2 mr-2" >
                                                ...
                                                <div className="card-body">
                                                    ...
                                                    <div className="chat pre-scrollable">
                                                        {chatMessages && chatMessages.map( message=> {
                                                            if (message.PublicProfileRandomNumber &&
                                                                (message.PublicProfileRandomNumber == profile.randomUnique)) {
                                                                return (
                                                                    <div key={message.PublicProfileRandomNumber}>
                                                                        <p>
                                                                            <strong>Recruiter:</strong>: {message.Message}
                                                                        </p>
                                                                    </div>
                                                                )
                                                            }
                                                    ...
                                                    <form>
                                                        <input type="text" placeholder="Say something nice"
                                                               ...
                                                               onChange={e => setSingleChatMessage({
                                                                   ID: 0,
                                                                   Message: e.target.value,
                                                                   CreatedAt: Date().toLocaleString(),
                                                                   FromUserID: 1,
                                                                   PublicProfileRandomNumber: profile.randomUnique,
                                                               })}/>
